% לדעתי יש בעיה שאחד הרובוטים עולה על המטרה של רובוט אחר 
...
Lx = 40;                             % plate width (m)
Ly = 4;                             % plate length (m)
nx = 180;                            % number of nodes in x direction
...
%% Initial Conditions
c_init = 100000 ;                   % Initial concentration in all nodes ( the whole plate )
c_bound = 100000 ;                 % Concentraiton on the boundary conditions ("Dirichlet Conditions" )
T.conc = -c_bound;
car_size = [2.5 1.5];
...
R.vinx(1,2:end) = R.vinx(1,2:end)+np.*(1:nT-1); % converting to vector shape requires the addition of values to the robots that are greater than 1     
...
while isoutTarget && it <= numberIterations %for a big number of nx, it sould be bigger for SS-convergence
    ...
    R.vreal(:,2:end) = R.vinx(:,2:end)-np.*(1:nT-1); 
...
cmap = cmapmat(:,:,:,end);
R.vinx(:,2:end) = R.vinx(:,2:end)-np.*(1:nT-1); % converting to vector shape requires the addition of values to the robots that are greater than 1     

%% Plot 2D Mesh
figure(1)
subplot(2,1,2)
plotMesh(X,Y,R,T,O,in_obs,in_robot,np,nT)
...
%% Functions
function [] = plotMesh(X,Y,R,T,O,in_obs,in_robot,np,nT)
hold on
plot(X(in_obs),Y(in_obs),'r+') % points inside
plot(X(~in_obs),Y(~in_obs),'b+') % points outside
plot(X(in_robot(1:np)),Y(in_robot(1:np)),'w+') % point inside the robot

if nT > 1
    plot(X(in_robot(np+1:2*np)),Y(in_robot(np+1:2*np)),'w+') % point inside the robot
end
...
for iplot = 1 : nT
    ...
    colormap('white')
    contourf(X,Y,cmap(:,:,iplot),[1,0.9999999999999998,0.999999999,0.99999999,0.9999999, 0.999999,  0.9999, 0.9990, 0.9900, 0.9000, 0.0100,0.0010,-0.3000, -1.0000]*c_bound)
